...
export default function (pam_data: any, version: number) {
    ...
    let resource = pam_data.readInt8();
    if (version >= 6 && resource == 255) {
        resource = pam_data.readInt16LE();
    }
    let preload_frames = 0;
    if ((number & 8192) != 0) {
        preload_frames = pam_data.readInt16LE();
    }
    let name = null;
    if ((number & 4096) != 0) {
    ...
    return {
        index, name, resource, sprite, additive, preload_frames, timescale
    }
}