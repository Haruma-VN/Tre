...
export default function (pam_change: any, version: number) {
    ...
    if (version >= 5) {
        changeInfo.writeInt32LE(pam_change.transform[pam_change.transform.length - 2] * 20);
        changeInfo.writeInt32LE(pam_change.transform[pam_change.transform.length - 1] * 20);
        flags += MoveFlags.LongCoords;
    }
    else {
        changeInfo.writeInt16LE(pam_change.transform[pam_change.transform.length - 2] * 20);
        changeInfo.writeInt16LE(pam_change.transform[pam_change.transform.length - 1] * 20);
    }
