...
interface TransformXDataResources {
    frameIdx: number,
    stop?: boolean,
}
export default function (reader: ReadBuffer, frameNum: number, isMainAnims: boolean) {
    let frames: Array<TransformXDataResources> = global.frames = new Array();
    for (var j: number = 0; j < frameNum; ++j) {
        frames.push({
            frameIdx: j + 1,
        });
        const currentFrame: any = frames[j];
        const key: number = reader.ReadByte();
        let canReadTrans: boolean = true;
        switch (key) {
            case 0x00:
                // Unknown ??
                break;
            ...
            default:
                if (canReadTrans) {
                    ...
                    for (let l: number = 0; l < transCount; ++l) {
