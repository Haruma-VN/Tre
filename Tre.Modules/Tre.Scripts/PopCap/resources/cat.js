...
export default function (dir = process.argv[2], mode, encode) {
    const result_from_read_dir = read_single_folder(dir);
    const rsg_data = result_from_read_dir.map((val) => {
    ...
    switch (mode) {
        case 2:
            for (let i = 0; i < rsg_data.length; i++) {
                let debug_json = { "id": "", "type": "simple", "parent": "", "res": "", "resources": [] };
    ...
    writejson('res.json', resources_output_result);
    return 0;
}