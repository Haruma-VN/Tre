...
export default async function (RSB_composite_files, TreRSBInfo, rsb_path, pool_index, composite_pool_index) {
    let rsgp_unpacked = new Array();
    let rsgp_list = new Array();
    let composite_list = new Array();
    let AutoPool_Info = new SmartBuffer();
    let RSGP_Data_Info = new SmartBuffer();
    let RSB_RSGP_Data = new SmartBuffer();
        ...
        for (let composite_folder of TreRSBInfo) {
            if (composite_folder[0] === composite_file) {
                ...
                for (let rsgp_item of composite_folder[1]) {
                    try {
                        let rsgp_file_data = new Object();
                        rsgp_file_data.Name = rsgp_item;
                        rsgp_file_data.Number = pool_index;
                        rsgp_list.push(rsgp_file_data);
                        const rsgp_data_info = readfilebuffer(`${rsb_path}/Packet/${rsgp_item}.rsgp`);
                        const rsgp_data_list = await get_rsgp_data(rsgp_data_info, pool_index, rsgp_item, ptx_before_number);
                        const rsgp_file = rsgp_data_list[0];
                ...
                ;
                let composite_file_data = new Object();
    ...
    ;
    function ConcatRSGP_ItemsPath(info_paths, folder_length, key_count, pos, pos_2) {
        let info_array = new Array();
        for (let i = 0; i < info_paths.length - 1; i++) {
            let info_temp = new Object();
            const folder_1 = info_paths[i].Name.toUpperCase();
            const folder_2 = info_paths[i + 1].Name.toUpperCase();
        ...
        for (let h = 0; h < info_paths.length - 1; h++) {
            const folder_1 = info_paths[h].Name.toUpperCase();
            const folder_2 = info_paths[h + 1].Name.toUpperCase();
            folder_1.length > folder_2.length ? folder_length = folder_1.length : folder_length = folder_2.length;
            for (let j = 0; j < folder_length; j++) {
                if (folder_1[j] !== folder_2[j]) {
    ...
    ;
    const PTX_Info = await get_ptx_info(rsgp_unpacked);
    rsgp_unpacked.push(JSON.parse('{"Name":""}'));
    rsgp_list.push(JSON.parse('{"Name":""}'));
    composite_list.push(JSON.parse('{"Name":""}'));
    rsgp_unpacked.sort(function (a, b) { if (a.Name.toUpperCase() < b.Name.toUpperCase()) {
        return -1;
    } if (a.Name.toUpperCase() > b.Name.toUpperCase()) {
        return 1;
    } return 0; });
    rsgp_list.sort(function (a, b) { if (a.Name.toUpperCase() < b.Name.toUpperCase()) {
        return -1;
    } if (a.Name.toUpperCase() > b.Name.toUpperCase()) {
        return 1;
    } return 0; });
    composite_list.sort(function (a, b) { if (a.Name.toUpperCase() < b.Name.toUpperCase()) {
        return -1;
    } if (a.Name.toUpperCase() > b.Name.toUpperCase()) {
        return 1;
    } return 0; });
    const FileList = ConcatRSGP_ItemsPath(rsgp_unpacked, 0, 0, 0, 0);
    const RSGPList = ConcatRSGP_ItemsPath(rsgp_list, 0, 0, 0, 0);
    const CompositeList = ConcatRSGP_ItemsPath(composite_list, 0, 0, 0, 0);
    const CompositeInfo = await get_composite_info(RSB_composite_files.sort(), TreRSBInfo, 0);
    let RSB_Header_Data_Length = 112 + FileList.length + RSGPList.length + CompositeInfo.length + CompositeList.length + RSGP_Data_Info.length + AutoPool_Info.length + PTX_Info.length;
