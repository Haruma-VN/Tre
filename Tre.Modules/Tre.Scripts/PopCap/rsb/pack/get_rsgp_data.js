...
export default async function (RSGP_data, pool_index, rsgp_file, ptx_before_number) {
    const file_data = SmartBuffer.fromBuffer(RSGP_data);
    const part0_Offset = file_data.readInt32LE(24);
    const part0_Size = file_data.readInt32LE(32);
    const part1_Size = file_data.readInt32LE(48);
    const info_size = file_data.readInt32LE(72);
    const info_offset = file_data.readInt32LE(76);
    const info_limit = info_size + info_offset;
    const rsgp_data = file_data.toBuffer().slice(16, 64);
    let rsgp_data_list = new Array();
    let rsgp_unpacked = new Array();
    let autopool_buffer = SmartBuffer.fromBuffer(Buffer.alloc(152));
    let rsgp_info_buffer = SmartBuffer.fromBuffer(Buffer.alloc(204));
    let ptx_number = 0;
    function Decompression(image, temp_offset, name_path) {
