...
export default async function (RSGP_items_packet_list: any[],
     headlength: number, rsb_path: string, pack_method: string, RSGP_file_data_list: any[]) {
    ...
    function GetDataRSGP(file_data: Buffer, rsgp_info: any) {
        ...
        if (rsgp_info.ptx_number != 0) {
            autopool_item_info.writeString(`${parse(rsgp_info.rsgp_name).name}${'_AutoPool'}`);
    ...
    for (let i = 0; i < rsgp_data_info_list.length; i++) {
        ...
        if (pack_method == 'simple') {
            ...
            else if (rsgp_data_info_list[i].rsgp_name.toUpperCase().indexOf('__MANIFESTGROUP__') != -1) {
                file_data = RSGP_items_packet_list[2][1];
            }
            else {
                file_data = fs_util.readfilebuffer(`${rsb_path}/Packet/${rsgp_data_info_list[i].rsgp_name}.rsgp`);
            }
        }
        else if (pack_method == 'everything') {
            for (let h = 0; h < RSGP_file_data_list.length; h++) {
                if (RSGP_file_data_list[h].rsgp_name.toUpperCase() == rsgp_data_info_list[i].rsgp_name.toUpperCase()) {
    ...
    return [rsgp_data_info.toBuffer(), autopool_info.toBuffer(), rsb_rsgp_filedata]
}