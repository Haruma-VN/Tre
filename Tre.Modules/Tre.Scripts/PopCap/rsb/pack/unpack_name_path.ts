...
export default async function (rsb_path: string, RSGP_items_list: any, pack_method: any, RSGP_file_data_list: any) {
    ...
    function Unpack_Packet(rsgp_data: any, composite_index: any, rsgp_name: any) {
        ...
        function WriteFile(name_path: string, temp_offset: number, atlas: boolean, ...any_any: number[]) {
            if (atlas) {
                ...
                const format = CheckPtxFormat(parseInt(((file_data_size / (image_width * image_height)) * 10 / 2) as any));
                ptx_number++;
                rsgp_packet_info.push({ name_path, composite_index, image_width, image_height, format });
            }
            else {
                rsgp_packet_info.push({ name_path, composite_index, image_width: 0, image_height: 0 });
            }
        };
        function Extract_File(atlas: boolean, temp_offset: number) {
            ...
            while (temp_offset < info_limit) {
                ...
                if (character_byte == '\x00') {
                    ...
                    atlas ? WriteFile(name_path, temp_offset += 12, true, temp_offset += 20) : WriteFile(name_path, temp_offset += 12, false);
        ...
        part1_ZSize != 0 ? Extract_File(true, info_offset) : Extract_File(false, info_offset);
    ...
    for (let rsgp_item of RSGP_items_list) {
        if (pack_method == 'simple' && rsgp_item.name_path.toUpperCase() == 'PACKAGES') {
            const packages_data = await rsgp_pack(`${rsb_path}/RES/PACKAGES`, true, true, false, true);
            Unpack_Packet(packages_data, rsgp_item.composite_index, rsgp_item.name_path);
            ...
            if (fs_util.if_file_exists(`${rsb_path}/Res/PROPERTIES/RESOURCES.res`)) {
                ...
                if (fs_util.if_file_exists(`${rsb_path}/Res/PROPERTIES/RESOURCES.json`)) {
                    resources_data = await rsgp_pack(`${rsb_path}/Res\\PROPERTIES\\RESOURCES.JSON`, true, true, true, true);
                }
                else if (fs_util.if_file_exists(`${rsb_path}/Res/PROPERTIES/RESOURCES.rton`)) {
            ...
            for (let h = 0; h < RSGP_file_data_list_clone.length; h++) {
                if (RSGP_file_data_list_clone[h].rsgp_name.toUpperCase() == rsgp_item.name_path.toUpperCase()) {
                    Unpack_Packet(RSGP_file_data_list_clone[h].rsgp_data, rsgp_item.composite_index, rsgp_item.name_path);
            ...
            Unpack_Packet(fs_util.readfilebuffer(`${rsb_path}/Packet/${rsgp_item.name_path}.rsgp`), rsgp_item.composite_index, rsgp_item.name_path);
        }
    }
