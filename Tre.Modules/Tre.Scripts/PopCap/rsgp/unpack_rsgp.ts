...
export default async function (file_input: string, arg: number): Promise<any> {
    ...
    function Unpack_Rsgp(offset: number, size: number, image: boolean, compression: boolean) {
            ...
            if (image === true) {
                ...
                async function Decode_PTX() {
                    switch (file_data_size / (image_width * image_height)) {
                        case 4:
                            console.log(color.fggreen_string(`◉ Execution Argument: Detected PopCap PTX Format 0. Is ${parse(name_path).base} format ARGB8888 for iOS`));
        ...
        try {
            ...
            TreErrorMessage({ error: "Native error occur from unpack execution", reason: "Unknown compression flag type", system: error.toString() }, "Unknown compression flag type");
            return;
        };
    }
