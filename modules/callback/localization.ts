...
function localization(locate_data: string): string {
    try {
        let language_json: any = readjson(process.cwd() + "/extension/settings/localization/" + (readjson(process.cwd() + "/extension/settings/toolkit.json", true) as any).language + ".json", true);
