...
function exception_encode_dimension(width: number, height: number): boolean {
    ...
    if (height < standard_dimension) {
        throw new Error(`${localization("the_height_is_too_small")}, this.height >= ${standard_dimension}`)
    }

    if (width < standard_dimension) {
        throw new Error(`${localization("the_width_is_too_small")}, this.width >= ${standard_dimension}`)
    }
