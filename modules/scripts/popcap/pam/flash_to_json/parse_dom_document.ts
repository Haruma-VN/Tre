...
export default function (dom_document: any, main_sprite: any) {
  ...
  if (
    typeof x_DOMTimeline.name !== "string" &&
  ...
  const DOMFrame_flow_list = x_DOMLayer_list[0].frames.DOMFrame;
  DOMFrame_flow_list.map(function (x_DOMFrame: any) {
  ...
  const DOMFrame_command_list = x_DOMLayer_list[1].frames.DOMFrame;
  DOMFrame_command_list.map(function (x_DOMFrame: any) {
    const frame_index = x_DOMFrame.index;
    let x_Actionscript = x_DOMFrame.Actionscript;
    if (x_Actionscript == undefined) {
      return;
