...
export default function (dir: string, mode: number, encode: any, res_array_for_data: any[], is_rewrite_mode: boolean = false, is_return_output_mode: boolean = false): any {
    let config_json: any = readjson(path.dirname(process.argv[1]) + "/extension/settings/toolkit.json", true);
    ...
    for (let i = 0; i < rsg_data.length; i++) {
        if ("resources" in rsg_data[i]) {
            for (let j: number = 0; j < rsg_data[i].resources.length; j++) {
                if (popcap_resource_pack_id_for_sprites.includes(rsg_data[i].resources[j].id)) {
                    rsg_data[i].resources[j].slot = popcap_resource_slot_for_sprites.at(popcap_resource_pack_id_for_sprites.indexOf(rsg_data[i].resources[j].id.toString() as string));
                }
                else {
                    rsg_data[i].resources[j].slot = slot_count;
                    if (rsg_data[i].resources[j].type === "Image" && (typeof rsg_data[i].resources[j].type === "string" || rsg_data[i].resources[j].type instanceof String)
                        && !(popcap_resource_pack_id_for_sprites.includes(rsg_data[i].resources[j].id)) && "id" in rsg_data[i].resources[j] && rsg_data[i].resources[j].id !== null
                        && rsg_data[i].resources[j].id !== undefined && rsg_data[i].resources[j].id !== void 0) {
                        popcap_resource_pack_id_for_sprites.push(rsg_data[i].resources[j].id);
