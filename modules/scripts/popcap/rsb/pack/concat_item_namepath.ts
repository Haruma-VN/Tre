...
export default async function (filepath: any) {
    function ConvertPathBuffer(path: string) {
        let itempath:SmartBuffer = SmartBuffer.fromOptions({ size: path.length });
    ...
    for (let i :number= 0; i < filepath.length - 1; i++) {
        ...
        const path_compare = await ConvertPathBuffer(filepath[i + 1].name_path.toUpperCase());
        ...
        for (let k: number = 0; k < path_length; k++) {
            if (filepath[i].name_path.toUpperCase()[k] != filepath[i + 1].name_path.toUpperCase()[k]) {
    ...
    for (let i: number = 0; i < filepath.length - 1; i++) {
        ...
        const path_length:number = path_orignal.length > path_compare.length ? path_orignal.length : path_compare.length;
        for (let k:number = 0; k < path_length; k++) {
