...
import { popcap_texture_decode, popcap_texture_encode } from "../library/img/texture/utility.js";
import { rsg_method } from "../scripts/popcap/rsg/util.js";
import { rsb_functions } from "../scripts/popcap/rsb/util.js";
import {
    res_pack,
    res_split,
    res_rewrite,
    ...
    switch (method as str) {
        case "popcap_rsb_unpack" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                rsb_functions.unpack_rsb(execute_file_dir);
            } else {
                execute_file_dir.forEach(async (file) => {
                    rsb_functions.unpack_rsb(file);
                });
            }
            break;
        case "popcap_rsb_resource_unpack" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                rsb_functions.unpack_rsb_resources(execute_file_dir);
            } else {
                execute_file_dir.forEach(async (file) => {
                    rsb_functions.unpack_rsb_resources(file);
                });
            }
            break;
        case "popcap_resource_to_res" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                popcap_resource_to_res.create_conversion(execute_file_dir, false);
            } else {
                execute_file_dir.forEach(async (file) => {
                    popcap_resource_to_res.create_conversion(file, false);
                });
            }
            break;
        ...
        case "popcap_merge_res_json" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                merge_res_json.create_conversion(execute_file_dir, false);
            } else {
                execute_file_dir.forEach(async (file) => {
                    merge_res_json.create_conversion(file, false);
                });
            }
            break;
        case "popcap_res_to_resource" as popcap_game_edit_method:
            ...
            break;
        case "popcap_rsg_unpack" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                rsg_method.unpack_rsg(execute_file_dir, fs_js.read_file(execute_file_dir, "buffer"), false);
            } else {
                execute_file_dir.forEach((file) => {
                    rsg_method.unpack_rsg(file, fs_js.read_file(file, "buffer"), false);
                });
            }
            break;
        case "popcap_rsg_unpack_simple" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                rsg_method.unpack_rsg(execute_file_dir, fs_js.read_file(execute_file_dir, "buffer"), true);
            } else {
                execute_file_dir.forEach((file) => {
                    rsg_method.unpack_rsg(file, fs_js.read_file(file, "buffer"), true);
                });
            }
            break;
        case "popcap_rsg_pack" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                rsg_method.pack_rsg(execute_file_dir, null, false);
            } else {
                execute_file_dir.forEach((file) => {
                    rsg_method.pack_rsg(file, null, false);
                });
            }
            break;
        case "popcap_rsg_pack_simple" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                rsg_method.pack_rsg(execute_file_dir, null, true);
            } else {
                execute_file_dir.forEach((file) => {
                    rsg_method.pack_rsg(file, null, true);
                });
            }
            break;
        case "batch_popcap_animation_add_media_content" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                ...
                execute_file_dir.forEach(async (file) => {
                    ...
                    fs_js.flash_anim_resize_notify();
                    const popcap_pam_resolution: number = Console.TextureQualityReadLine();
                    await add_content(images_add, file, popcap_pam_resolution as 1536 | 768 | 384 | 640 | 1200, false);
                });
            }
            break;
        ...
        case "popcap_atlas_split" as popcap_game_edit_method:
            if (js_checker.is_array(execute_file_dir)) {
                ...
                atlas_split(atlas_split_method as 1 | 2, execute_file_dir);
            }
            break;
        ...
        case "json_patch" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                ...
                fs_js.write_json(
                    `${fs_js.dirname(`${json_apply_path}`)}/${fs_js.parse_fs(json_apply_path).name}.patched.json`,
        ...
        case "json_patch_generator" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                ...
                fs_js.write_json(
                    `${fs_js.dirname(`${json_new_file_compare_diff}`)}/${
        ...
        case "real_esrgan_upscaler_image" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir) && !check_if_the_directories_iz_folder) {
                ...
                await ImagesUtilities.real_esrgan(
                    ...
                    upscale_model,
                    upscale_data,
                    `${fs_js.dirname(`${execute_file_dir}`)}/${
            ...
            break;
        case "popcap_texture_decode" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                ...
                execute_file_dir.forEach((file) => {
                    ...
                    Console.WriteLine(color.fgcyan_string(`${Argument.Tre.Packages.decode_height}`));
                    height = Console.IntegerReadLine(1, 16384);
                    popcap_texture_decode(fs_js.read_file(file, "buffer"), file, width, height, -1, false, false);
                });
            }
            break;
        case "popcap_texture_encode" as popcap_game_edit_method:
            if (!js_checker.is_array(execute_file_dir)) {
                popcap_texture_encode(fs_js.read_file(execute_file_dir, "buffer"), execute_file_dir, -1, false, false);
            } else {
                execute_file_dir.forEach((file) => {
                    popcap_texture_encode(fs_js.read_file(file, "buffer"), file, -1, false, false);
                });
            }
            break;
        default:
