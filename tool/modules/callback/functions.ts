...
export default async function (
    ...
    execute_file_count = js_checker.is_array(execute_file_dir)
        ? execute_file_dir.length
        : execute_file_count;
    execute_file_length = js_checker.is_array(execute_file_dir)
        ? execute_file_dir.length
        : execute_file_length;
    ...
    if (js_checker.is_array(execute_file_dir)) {
            ...
            if (json_config.display.disable_display_full_path_execution) {
                if (index === execute_file_dir.length - 1) {
                    Console.WriteLine(
                        `     ${fs_js.basename(
                            child_file_bundle
                        )} | ${check_evaluate_system(child_file_bundle)}\n`
                    );
                } else {
                    Console.WriteLine(
                        `     ${fs_js.basename(
                ...
                if (index === execute_file_dir.length - 1) {
                    Console.WriteLine(
                        `     ${child_file_bundle} | ${check_evaluate_system(
                            child_file_bundle
                        )}\n`
                    );
                } else {
        ...
        if (json_config.display.disable_display_full_path_execution) {
            Console.WriteLine(
                `     ${fs_js.basename(
                    execute_file_dir
                )} | ${check_evaluate_system(execute_file_dir)}\n`
            );
        } else {
            Console.WriteLine(
                `     ${execute_file_dir} | ${check_evaluate_system(
                    execute_file_dir
                )}\n`
            );
        }
    ...
    Console.WriteLine(
        color.fggreen_string(`${Argument.Tre.Packages.module_available}`)
    );
    const generate_modules =
        Display.ToolKit.Function.create_new_functions_view.sort((a, b) => {
            return (
                a.void_number_readline_argument() -
                b.void_number_readline_argument()
            );
        });
    generate_modules.forEach((module_on_waiting_list) => {
        if (
            !js_checker.is_array(execute_file_dir) &&
            (module_on_waiting_list
                .static_filter()
                .includes(fs_js.js_extname(execute_file_dir, true)) ||
            ...
            Display.ToolKit.Function.DisplayItems(
                user_selector_available_list,
                module_on_waiting_list
            );
        } else if (
            !js_checker.is_array(execute_file_dir) &&
    ...
    if (user_selector_available_list.length === 0) {
        Console.WriteLine(
            color.fgred_string(
                `${Argument.Tre.Packages.cannot_load_any_modules}`
            )
        );
    } else {
        const option: number = Console.ExpandReadLine(
            user_selector_available_list
        );
        if (typeof execute_file_dir === "string") {
            ...
            option !==
                Display.ToolKit.Function.popcap_texture_atlas_split.void_number_readline_argument() &&
            option !==
                Display.ToolKit.Function.popcap_atlas_split_advanced.void_number_readline_argument()
        ) {
