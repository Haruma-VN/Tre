...
import get_extension from "./public/extension/extension.js";
...
export default async function (execute_file_count: number, execute_file_dir: string | Array<string>, execute_file_length: number): Promise<void> {
    ...
    if (js_checker.is_array(execute_file_dir)) {
        execute_file_dir.forEach((child_file_bundle: string, index: number) => {
            if (json_config.display.disable_display_full_path_execution) {
                if (index === execute_file_dir.length - 1) {
                    Console.WriteLine(`     ${fs_js.basename(child_file_bundle)} | ${check_evaluate_system(child_file_bundle)}\n`);
                } else {
                    Console.WriteLine(`     ${fs_js.basename(child_file_bundle)} | ${check_evaluate_system(child_file_bundle)}\n`);
                }
            } else {
        ...
        if (json_config.display.disable_display_full_path_execution) {
            Console.WriteLine(`     ${fs_js.basename(execute_file_dir)} | ${check_evaluate_system(execute_file_dir)}\n`);
        } else {
        ...
        if (
            ...
            module_on_waiting_list
                ...
                .some(
                    (e: string) =>
                        (e.startsWith(".")
                            ? e.toLowerCase().endsWith(get_extension(fs_js.basename(execute_file_dir as string).toLowerCase()))
                            : (execute_file_dir as string).toLowerCase().startsWith(e.toLowerCase())) || e.toLowerCase() === "~*/",
                )
        ) {
    ...
    if (user_selector_available_list.length === 0) {
        ...
        if (typeof execute_file_dir === "string") {
            ...
            option !== Display.ToolKit.Function.popcap_texture_atlas_split.void_number_readline_argument() &&
