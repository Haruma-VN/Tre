...
import { EvaluateWithoutDefaultPoint } from "../../implement/error.js";
/**
 *
...
async function evaluate_script(script_entry_point: evaluate_script) {
    ...
    for (let index: number = 0; index < script_entry_point.modules.length; ++index) {
    ...
    if (evaluate_end === -1) {
        throw new EvaluateWithoutDefaultPoint(`${localization("no_end_point")}`, "end");
    }
    if (evaluate_start === -1) {
        throw new EvaluateWithoutDefaultPoint(`${localization("no_start_point")}`, "start");
    }
    for (let index: number = evaluate_start; index <= evaluate_end; ++index) {
        ...
        if (
            ...
            fs_js.execution_information(script_entry_point.modules[index].notify);
        }
        ...
        if (
            ...
            !assertation_break.ban_list.includes(script_entry_point.modules[index].func as string) &&
            assertation_break.allowance_lists.includes(script_entry_point.modules[index].func as string)
        ) {
            fs_js.execution_auto(localization(script_entry_point.modules[index].func as string));
        ...
        if (
            ...
            evaluate_modules: for (let evaluate_file of script_entry_point.modules[index].entry as Array<string>) {
                try {
                    switch (evaluate_file as string) {
                        ...
                        case "?":
                            ...
                            (evaluate_file as string) = await Console.ReadPath();
                        ...
                        case "?file":
                            ...
                            (evaluate_file as string) = await Console.ReadFile();
                            await evaluate_modules_workspace_assertation(
                                evaluate_file as string,
                                script_entry_point.modules[index].func as string,
                            );
                        ...
                        default:
                            if (!fs_js.js_exists(evaluate_file)) {
                                Console.Error(
                                    `${localization("cannot_find_the_target_file")} ${fs_js.get_full_path(
                                        evaluate_file,
                                    )}`,
                                );
                            }
