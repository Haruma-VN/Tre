...
export default async function processFilePaths(
    ...
    let filePath = is_windows_explorer_open
    ...
    while (filePath !== "") {
        ...
        try {
            ...
            if (
                ...
                !filePath
                    .toLowerCase()
                    .endsWith(`.${file_type_but_only_extension_name}`)
            ) {
                console.error(
                    `${filePath} ${localization(
                        "is_a_directory_not_a_valid_any_file"
                    )} ${executor_file_need_avoid} ${localization("file")}`
                );
                filePath = is_windows_explorer_open
                    ? await fs_js.openWindowsExplorer("file", extension)
                    : Console.ReadLine();
