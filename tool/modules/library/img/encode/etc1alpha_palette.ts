...
export default async function (dir: string, not_notify_console_log: boolean = false): Promise<void> {
    ...
    const etc_process: string = `etcpak.exe --etc1 "${dir}" "${fs_js.dirname(dir)}/${fs_js
        .parse_fs(dir)
        .name.toUpperCase()}.ptx"`;
    ...
    if (!not_notify_console_log) {
        Console.WriteLine(color.fggreen_string(`◉ ${localization("execution_information")}: `) + "rgb_etc1_a_palette");
        Console.WriteLine(
        ...
        await sharp(dir)
            ...
            .then((pixels: any) => {
                const originalImage: Buffer = fs_js.read_file(
                    `${fs_js.dirname(dir)}/${fs_js.parse_fs(dir).name.toString().toUpperCase()}.ptx`,
                    "buffer",
                );
                ...
                fs_js.write_file(`${fs_js.dirname(dir)}/${fs_js.parse_fs(dir).name.toUpperCase()}.ptx`, etc_image);
                if (!not_notify_console_log) {
                    Console.WriteLine(
                        color.fggreen_string(`◉ ${localization("execution_out")}:\n     `) +
                            `${fs_js.resolve(dir + "/../" + fs_js.parse_fs(dir).name.toString() + ".ptx")}`,
                    );
                }
                for (let item of fs_js.full_reader(tre_thirdparty)) {
                    fs_js.extname(item).toUpperCase() !== ".EXE" ? fs_js.js_remove(`${tre_thirdparty}${item}`) : {};
                }
            });
    }
}
