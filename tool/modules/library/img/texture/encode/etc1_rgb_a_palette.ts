...
export default function encode_etc1_rgb_palette(img_buf: Buffer): Buffer {
    ...
    function encode_image(width: number, height: number, img_c: any): Buffer {
        ...
        image_b.writeUInt8(0x10);
        for (let i = 0; i < 16; i++) {
            image_b.writeUInt8(i);
        };
        let img_s: number = width * height;
        const odd: boolean = (img_s & 0b1) === 1;
        img_s >>= 1;
        for (let i = 0; i < img_s; i++) {
            image_b.writeUInt8((pixels[(i << 1) * 4 + 3] & 0b11110000) | (pixels[((i << 1) | 1) * 4 + 3] >> 4));
        }
        if (odd) image_b.writeUInt8((pixels[(img_s << 1) * 4 + 3] & 0b11110000))
