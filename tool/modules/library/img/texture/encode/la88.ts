...
export default function encode_la88(img_buf: Buffer): Buffer {
    ...
    function encode_image(pixels: Uint8ClampedArray, img_s: number): Buffer {
        ...
        for (let i = 0; i < img_s; i ++) {
            image_b.writeUInt16LE((((Math.max(pixels[i * 4] * 0.299 + pixels[i * 4 + 1] * 0.587 + pixels[i* 4 + 2] * 0.114, 255)) << 8) | pixels[i * 4 + 3]));
        }
