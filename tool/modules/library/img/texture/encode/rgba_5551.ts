...
export default function encode_rgba_5551(img_buf: Buffer): Buffer {
    ...
    function encode_image(pixels: Uint8ClampedArray, img_s: number): Buffer {
        ...
        for (let i = 0; i < img_s; i ++) {
            image_b.writeUInt16LE(((pixels[i * 4 + 3] & 0x80) >> 7) | ((pixels[i * 4 + 2] & 0xF8) >> 2) | ((pixels[i * 4 + 1] & 0xF8) << 3) | ((pixels[i * 4] & 0xF8) << 8));
        }
