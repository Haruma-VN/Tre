...
function prompt(question: string = "\x1b[36m◉ ", range?: any,
    ...
    if (use_answer_as_exception_message) {
        exception_range_message = color.fgred_string(`◉ ${localization("execution_error")}: ${answer} ${exception_message_here}`);
        not_in_range = color.fgred_string(`◉ ${localization("execution_error")}: ${answer} ${exception_message_here}`);
    }
