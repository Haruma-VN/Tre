    ...
    if (atlasinfo.groups != undefined && atlasinfo.groups != null && atlasinfo.groups != void 0) {
        if ("groups" in atlasinfo) {
            for (let i: number = 0; i < atlasinfo?.groups.length; ++i) {
                switch (method) {
                    case "path":
                        writejson(resources_atlas_folder + "/" + atlasinfo.groups[i].path[atlasinfo.groups[i].path.length - 1] + ".json", atlasinfo.groups[i]);
                        break;
                    default:
                        writejson(resources_atlas_folder + "/" + atlasinfo.groups[i].id + ".json", atlasinfo.groups[i]);
                        break;
                };
            };
        }
