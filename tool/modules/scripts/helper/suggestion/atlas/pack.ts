...
async function evaluate_test(
    ...
    const create_evaluate_array: string[] = fs_js.one_reader(file_system_directory);
    ...
    const create_new_dimension_array: Array<Promise<{
        width: number,
        height: number,
    }>> = await Promise.all(
        create_evaluate_array.map(async (evaluate_file: string) => {
            if (fs_js.js_check_extname(evaluate_file, ".png")) {
                return (await (fs_js.get_dimension(`${file_system_directory}/${evaluate_file}`) as Promise<{
                    width: number,
                    height: number,
                }>).then((result: {
                    width: number,
    ...
    ).then(result => result.filter((dimension) => (dimension as {
        width: number,
        height: number,
        filename: string,
    }) != undefined && dimension != null && dimension != void 0)) as Array<
        Promise<{
            width: number,
            height: number,
            filename: string,
        }>>;
    const max_number_in_dimension_array = await create_new_dimension_array.reduce(
        async (acc, current) => {
    ...
    const max_number_compare: number = Math.max(max_number_in_dimension_array.maxHeight, max_number_in_dimension_array.maxHeight);
    ...
    for (let i: number = 0; i < test_case_for_max_rects_bin.length; i++) {
        ...
        for (let j: number = 0; j < test_case_for_max_rects_bin.length; j++) {
            let packer = await new MaxRectsPacker(await test_case_for_max_rects_bin[i], await test_case_for_max_rects_bin[j], padding, options);
