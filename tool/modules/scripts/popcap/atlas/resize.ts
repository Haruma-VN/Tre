...
export default async function (
    ...
    const new_set_entry_point: string = `${dir}/../${path
        .parse(dir)
        .name.replace(`_${orig.toString()}`, `_${mod.toString()}`)}.spg`;
    let upscaler_model_default: string =
        parseInt(calculator.toString()) === 4
    ...
    for (let i: number = 0; i < files_process.length; ++i) {
        if (
            path.extname(`${dir}/${files_process[i]}`).toLowerCase() === ".json"
        ) {
            if (
                path.parse(`${dir}/${files_process[i]}`).name.toLowerCase() ===
                "atlasinfo"
            ) {
            ...
            path.extname(`${dir}/${files_process[i]}`).toLowerCase() === ".png"
        ) {
            if (calculator >= 1) {
                await resize(
                    `${dir}/${files_process[i]}`,
                    orig,
                    mod,
                    `${new_set_entry_point}/${files_process[i]}`
                ).finally(() => {});
            } else {
                await real_esrgan(
                    `${dir}`,
                    upscaler_model_default,
                    parseInt((1 / calculator).toString()),
                    `${new_set_entry_point}`
                );
