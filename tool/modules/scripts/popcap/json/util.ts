...
namespace PopCapPackages.Json {
    ...
    export function Split(
        popcap_common_json_file_location: string,
        popcap_split_method_selector: number
    ): void {
        ...
        const popcap_common_json = ReadJSONObject(
            popcap_common_json_file_location
        );
        ...
        if (popcap_common_json.objects !== undefined) {
            fs_js.create_directory(
                directory_contain_popcap_splitable_data,
                true
            );
            Console.WriteLine(
            ...
            for (
                let i: number = 0;
                i < popcap_common_json.objects.length;
                i++
            ) {
                switch (popcap_split_method_selector) {
                    case 1:
                        if (popcap_common_json.objects[i] !== undefined) {
                            if (
                                popcap_common_json.objects[i].aliases !==
                                undefined
                            ) {
                                if (
                                    popcap_common_json.objects[i].aliases[0] !==
                                    undefined
                                ) {
                                    fs_js.write_json(
                    ...
                    case 2:
                        if (
                            popcap_common_json.objects[i].aliases !== undefined
                        ) {
                            if (
                                popcap_common_json.objects[i].objdata
                                    ?.TypeName !== undefined
                            ) {
                                fs_js.write_json(
    ...
    export function Cat(
        ...
        const popcap_in_common_jsons_list: string[] = fs_js.one_reader(
            popcap_common_directory_file_location
        );
        const popcap_common_json_list: PopCapCommonJSONObject[] = new Array();
        popcap_in_common_jsons_list.forEach(function (
            popcap_json_file_directory: string
        ) {
            const popcap_common_json_object = fs_js.read_json(
                popcap_json_file_directory
            ) as PopCapCommonJSONObject;
    ...
    export function CatToFile(
        ...
        file_out_type === "json"
            ? fs_js.write_json(output, create_popcap_common_json_object)
            : fs_js.write_file(
                  output,
                  json2rton(create_popcap_common_json_object)
              );
        return;
    }
}
