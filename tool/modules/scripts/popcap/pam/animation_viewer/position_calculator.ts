...
export default function position_calculator(pam_json: any) {
    ...
    function pushSpritePositon(sprite: any) {
        for (let frame of sprite.frame) {
            for (let move of frame.change) {
                ...
                y_transformArray.push(move.transform[move.transform.length - 1]);
            }
        }
    }
    function getMin(arr: any) {
        return arr.reduce((min: number, v: number) => (min <= v ? min : v), Infinity);
    }
    ...
    const x_tranform: number = Math.abs(Math.floor(getMin(x_tranformArray) / 2));
    const y_transform: number = Math.abs(Math.floor(getMin(y_transformArray) / 2));
