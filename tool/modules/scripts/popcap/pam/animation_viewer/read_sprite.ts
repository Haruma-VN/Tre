...
export default async function (
    ...
    const frame_node_list: any = write_sprite_xlf(sprite, animation_sprite, index, true);
    ...
    for (const layer_index in frame_node_list) {
        ...
        for (const frame_object of DOMLayer) {
            ...
            for (let k = 0; k < duration; k++) {
                const DOMSymbolInstance: any = frame_object.elements.DOMSymbolInstance;
                if (DOMSymbolInstance != undefined) {
                    const frame_index_duration: number = frame_index + k;
                    const name_match: string = DOMSymbolInstance.libraryItemName.match(/(image|sprite)\/(image|sprite)_([0-9]+)/);
                    const resource = Number(name_match[3]) - 1;
                    const sprite_type: boolean = name_match[2] == "sprite";
                    const transform: Array<number> = parse_transform(DOMSymbolInstance.matrix.Matrix);
                    const color: Array<number> = parse_color(DOMSymbolInstance.color.Color).map((color_channel) => color_channel * 255);
                    const frame_sprite_list = sprite_type ? structuredClone(sprite_list[resource]) : structuredClone(image_list[0][resource]);
                    if (frame_composite_mapping[frame_index_duration] == undefined) {
                        frame_composite_mapping[frame_index_duration] = new Array();
                    }
                    for (let i = 0; i < frame_sprite_list.length; i++) {
                        const frame_sprite: any = frame_sprite_list[i];
                        if (sprite_type && sprite_could_disable.includes(resource + 1) && sprite_could_disable.length > 0) {
                            frame_sprite.sprite_disable = true;
                        }
                        frame_sprite.transform = frame_sprite.transform ?? new Array();
                        ...
                        if (index !== -1) {
                            ...
                            maxWidthArray.push(frame_sprite.image_width + transform[4]);
                            maxHeightArray.push(frame_sprite.image_height + transform[5]);
                            frame_composite_mapping[frame_index_duration].push(frame_sprite);
                        }
                    }
                }
            }
    ...
    if (index !== -1) {
        ...
        const max_square: any = {
            max_width: Math.round(getMax(maxWidthArray) + max_position[0]+ setting.width_append),
            max_height: Math.round(getMax(maxHeightArray) + max_position[1] + setting.height_append),
        };
        ...
        await make_png(frame_composite_mapping, image_list, max_position, max_square, DOM_main_sprite, apng_folder_path, frame_rate, setting);
    }
}
