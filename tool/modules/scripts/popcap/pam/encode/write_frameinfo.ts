...
export default function (pam_frame: any, version: number): Buffer {
    ...
    let flags = 0;
    if (pam_frame.remove !== null && pam_frame.remove.length > 0)
        flags += FrameFlags.Removes;
    if (pam_frame.append !== null && pam_frame.append.length > 0)
        flags += FrameFlags.Adds;
    if (pam_frame.change !== null && pam_frame.change.length > 0)
        flags += FrameFlags.Moves;
    if (pam_frame.label !== null) flags += FrameFlags.FrameName;
    if (pam_frame.stop) flags += FrameFlags.Stop;
    if (pam_frame.command !== null && pam_frame.command.length > 0)
    ...
    if ((flags & FrameFlags.Adds) !== 0) {
        ...
        for (let i = 0; i < append_count; i++) {
            frameInfo.writeBuffer(
                writeAppendInfo(pam_frame.append[i], version)
            );
        }
    }
    ...
    if ((flags & FrameFlags.FrameName) !== 0) {
        frameInfo.writeUInt16LE(pam_frame.label.length);
