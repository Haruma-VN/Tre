...
export default function (pam_image: any, imagesCount: number, version: number): Buffer {
    const imageInfo = new SmartBuffer();
    for (let i = 0; i < imagesCount; i++) {
        const name = pam_image[i].name;
        imageInfo.writeInt16LE(name.length);
        ...
        if (version == 1) {
            if (pam_image[i].transform == null && pam_image[i].transform.length < 2) {
                ...
                if (pam_image[i].transform[1] * ((version == 2 as any)? -1 : 1) < 0) {
                    Rcos = -Rcos;
                }
