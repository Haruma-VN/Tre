...
export default function (folder_input: string, texture_reslution: number) {
    ...
    source_list.forEach(function(source: any) {
        let xml_source = XMLMapping.load(xmlButPrettier(fs_js.read_file(source, 'utf8')));
        let image_reslution = xml_source.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer.frames.DOMFrame.elements.DOMBitmapInstance.matrix.Matrix;
