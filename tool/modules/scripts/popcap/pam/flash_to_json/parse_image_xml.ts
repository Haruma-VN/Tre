...
export default function (image_xml: any, index: number, item: any) {
    ...
    if (
        image_xml.DOMSymbolItem.timeline.DOMTimeline.name !==
        `image_${index + 1}`
    ) {
        throw new Error(image_name + " - image_invalid_name_timeline");
    }
    if (
        Object.keys(image_xml.DOMSymbolItem.timeline.DOMTimeline.layers)
            .length !== 1
    ) {
    ...
    if (
        Object.keys(
            image_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer
        ).length !== 1 &&
        Object.keys(
            image_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer
        ).length !== 4
    ...
    if (
        Object.keys(
            image_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer.frames
        ).length !== 1
    ) {
    ...
    if (
        image_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer.frames
            .DOMFrame.elements.DOMSymbolInstance.libraryItemName !==
    ...
    if (
        Object.keys(
            image_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer.frames
                .DOMFrame.elements.DOMSymbolInstance.matrix
        ).length !== 1
    ) {
    ...
    if (
        Object.keys(
            image_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer.frames
                .DOMFrame.elements.DOMSymbolInstance.transformationPoint
        ).length !== 1
    ) {
    ...
    const matrix =
        image_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer.frames
            .DOMFrame.elements.DOMSymbolInstance.matrix.Matrix;
    const transform_point =
        image_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer.frames
            .DOMFrame.elements.DOMSymbolInstance.transformationPoint.Point;
    const transform = parse_transform(matrix);
    const transform_origin = parse_transform_origin(transform_point);
    if (
        transform[4] !== -transform_origin[0] ||
        transform[5] !== -transform_origin[1]
    ) {
        throw new Error(image_name + " - image_invalid_matrix");
    } else {
        return {
