...
export default function (
    ...
    if (
        ...
        throw new Error(sprite_name + localization("sprite_invalid_name"));
    }
    if (Object.keys(sprite_xml.DOMSymbolItem.timeline).length !== 1) {
        throw new Error(sprite_name + localization("sprite_timeline_invalid"));
    }
    if (
        sprite_xml.DOMSymbolItem.timeline.DOMTimeline.name !==
        (index === -1 ? "main_sprite" : `sprite_${index + 1}`)
    ) {
        throw new Error(
            sprite_name + localization("sprite_invalid_name_timeline")
        );
    }
    if (
        Object.keys(sprite_xml.DOMSymbolItem.timeline.DOMTimeline.layers)
            .length !== 1
    ) {
        throw new Error(sprite_name + localization("sprite_invalid_layers"));
    }
    let model: any = null;
    let result: any[] = new Array();
    let DOMlayer_list: any[] = new Array();
    if (
        Array.isArray(
            sprite_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer
        )
    ) {
        DOMlayer_list =
            sprite_xml.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer;
    ...
    function close_current_model() {
        if (model !== null) {
            let target_frame = get_frame_at_index(
                model.frame_start + model.frame_duration
            );
    ...
    DOMlayer_list.forEach(function (x_DOMLayer) {
        if (Object.keys(x_DOMLayer.frames).length !== 1) {
            throw new Error(sprite_name + " - sprite_invalid_frames");
        }
        ...
        x_DOMFrame_list.forEach(function (x_DOMFrame) {
            ...
            const frame_duration = parseInt(
                Number(x_DOMFrame.duration ?? 1) as any
            );
            ...
            if (Object.keys(x_elements_list).length !== 1) {
                throw new Error(sprite_name + " - sprite_invalid_elements");
            }
            const x_DOMSymbolInstance_list = [
                x_elements_list.DOMSymbolInstance,
            ];
            ...
            if (x_DOMSymbolInstance_list.length !== 1) {
                throw new Error(
                    sprite_name + " - sprite_invalid_DOMSymbolInstance"
                );
            }
            const x_DOMSymbolInstance = x_DOMSymbolInstance_list[0];
            const name_match = x_DOMSymbolInstance.libraryItemName.match(
                /(image|sprite)\/(image|sprite)_([0-9]+)/
            );
            if (name_match[0] === undefined) {
                throw new Error(sprite_name + " - sprite_invalid_name");
            }
            if (name_match[1] !== name_match[2]) {
                throw new Error(sprite_name + " - sprite_invalid_name_x");
            }
            ...
            {
                if (
                    x_DOMSymbolInstance.matrix === undefined ||
                    Object.keys(x_DOMSymbolInstance.matrix).length === 0
                ) {
                    transform = [0, 0];
                } else if (
                    Object.keys(x_DOMSymbolInstance.matrix).length === 1
                ) {
                    const x_matrix = [x_DOMSymbolInstance.matrix.Matrix];
                    if (x_matrix.length !== 1 || x_matrix[0] === undefined) {
                        throw new Error(
                            sprite_name + " - sprite_invalid_matrix"
                        );
                    }
                    transform = transformCalculator(
                        parse_transform(x_matrix[0])
                    );
                } else {
                    throw new Error(sprite_name + " - sprite_invalid_append");
                }
            }
            {
                if (
                    x_DOMSymbolInstance.color === undefined ||
                    Object.keys(x_DOMSymbolInstance.color).length === 0
                ) {
                    color = [0, 0];
                } else if (
                    Object.keys(x_DOMSymbolInstance.color).length === 1
                ) {
                    const x_color = [x_DOMSymbolInstance.color.Color];
                    if (x_color[0] === undefined || x_color.length !== 1) {
                        throw new Error(
                            sprite_name + " - sprite_invalid_color"
                        );
                    }
                    color = parse_color(x_color[0]);
                } else {
                    throw new Error(sprite_name + " - sprite_invalid_color");
                }
            }
            ...
            if (model === null) {
                ...
                if (
                    current_instance.resource !== model.resource ||
                    current_instance.sprite !== model.sprite
                ) {
                    throw new Error(sprite_name + " - sprite_invalid_model");
                }
            }
