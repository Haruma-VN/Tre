...
export default function (animation_json: any) {
  ...
  animation_json.main_sprite.frame.map(function (frame: any, frame_index: any) {
    ...
    if (frame.command.length > 0) {
      ...
      frame.command.map(function (item: any) {
        commands.push(`fscommand(\"${item[0]}\", \"${item[1]}\");`);
      });
  ...
  if (prev_end.flow + 1 < animation_json.main_sprite.frame.length) {
    flow_node.DOMFrame.push({
      index: prev_end.flow + 1,
      duration: animation_json.main_sprite.frame.length - (prev_end.flow + 1),
    });
  }
  if (prev_end.command + 1 < animation_json.main_sprite.frame.length) {
    command_node.DOMFrame.push({
      index: prev_end.command + 1,
      duration: animation_json.main_sprite.frame.length - (prev_end.command + 1),
    });
  }
  ...
  animation_json.image.map(function (item: any, index: number) {
    DOMBitmapItem.push({
  ...
  animation_json.sprite.map(function (name: string, index: number) {
    IncludeSprite.push({
  ...
  const dom_document = {
    DOMDocument: {
      ...
      frameRate: animation_json.main_sprite.frame_rate,
      width: animation_json.size[0],
      height: animation_json.size[1],
      xflVersion: "2.971",
            ...
            DOMLayer: [
              ...
              },
              {
                ...
                frames: {
                  DOMFrame: {
                    index: "0",
                    duration: animation_json.main_sprite.frame.length,
                    elements: {
                      DOMSymbolInstance: {
