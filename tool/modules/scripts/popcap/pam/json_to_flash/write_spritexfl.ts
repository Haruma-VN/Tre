...
export default function (sprite: any, sub_sprite: any, index: number, return_mode: boolean = false) {
          ...
          : {
            ...
            elements: {
              DOMSymbolInstance: {
                ...
                symbolType: "graphic",
                loop: "loop",
                firstFrame: (frame_index - layer.frame_start) % sub_sprite[layer.resource].frame.length,
                matrix: {
                  Matrix: {
                    a: layer.transform[0].toFixed(6),
                    b: layer.transform[1].toFixed(6),
  ...
  sprite_xml.DOMSymbolItem.name = index == -1 ? "main_sprite" : `sprite/sprite_${index + 1}`;
  let DOMLayerSprite: any = new Object({ DOMLayer: [] });
  Object.keys(frame_node_list)
    .sort((a: auto, b: auto) => b - a)
    .map((layer_index) => {
