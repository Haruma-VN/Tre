...
export async function popcap_pam_to_flash(file_system_data_input_argument: string, texture_reslution: number) {
    await pamjson2pamflash(
        await read_pam(await fs_js.read_file(file_system_data_input_argument, "buffer")),
        file_system_data_input_argument,
        texture_reslution,
    );
}

export async function popcap_flash_to_pam(file_system_data_input_argument: string) {
...
export async function frame_rate_increasement(file_system_data_input_argument: string, ratio: 2 | 3 | 4) {
...
export async function popcap_pam_json_to_flash(file_system_data_input_argument: string, texture_reslution: number) {
    await pamjson2pamflash(
...
export async function flash_animation_resize(file_system_data_input_argument: string, texture_reslution: number) {
    await fs_resize(file_system_data_input_argument, texture_reslution);
    fs_js.execution_finish(localization("all_flash_resized"));
}

export async function popcap_flash_to_pam_json(file_system_data_input_argument: string) {
    const pam_json = await pamflash_to_pamjson(file_system_data_input_argument);
