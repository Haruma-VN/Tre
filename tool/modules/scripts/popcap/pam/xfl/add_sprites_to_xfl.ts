...
export default async function (path: string, number_sprites: number) {
    ...
    let xml_source = XMLMapping.load(
        xmlButPrettier(
            fs_js.read_file(`${path}/LIBRARY/source/source_1.xml`, "utf8")
        )
    );
    const image_reslution =
        xml_source.DOMSymbolItem.timeline.DOMTimeline.layers.DOMLayer.frames
            .DOMFrame.elements.DOMBitmapInstance.matrix.Matrix.a;
    ...
    for (let k = 0; k < filter_source_folder.length; k++) {
        if (!image_list.includes(filter_source_folder[k])) {
            ...
            const source_dimension = await dimension(
                `${path}/LIBRARY/media/${filter_source_folder[k]}.png`
            );
            const image: {
                name: string;
                size: [number, number];
                ...
                name: `${
                    filter_source_folder[k]
                }|${resource_build.extend_id.toUpperCase()}${filter_source_folder[
                    k
                ].toUpperCase()}`,
            ...
            fs_js.outfile_fs(
                `${path}/LIBRARY/source/source_${image_count + 1}.xml`,
            ...
            fs_js.outfile_fs(
                `${path}/LIBRARY/image/image_${image_count}.xml`,
    ...
    const sprite_list = createSprite(
        sprite_length,
        sprite_count + 1,
        "blank_sprite_"
    );
    ...
    for (let h = sprite_count; h < sprite_length; h++) {
        ...
        fs_js.outfile_fs(
            `${path}/LIBRARY/sprite/sprite_${h + 1}.xml`,
