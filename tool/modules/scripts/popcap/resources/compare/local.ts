...
export default function LocalResourcesCompare(
    ...
    const output_copy_packet_resource: string[] = LocalCompareItemsInDirectory(
        res2,
        same_items_in_res
    );
    for (const res_data_in_original_packet in res1) {
        for (const res_data_in_modified_packet in res2) {
            if (
                ...
                if (
                    JSON.stringify(file_origin_vsdiff) ===
                    JSON.stringify(file_modified_vsdiff)
                ) {
                    continue;
                } else {
                    output_copy_packet_resource.push(
                        res2[res_data_in_modified_packet]
                    );
                }
            }
