        ...
        if (
            ...
            for (
                ...
                if (
                    ...
                    const path_iz_list: boolean = Array.isArray(
                        res_json_full.groups[i].resources[j].path
                    );
                    if (!path_iz_list) {
                        res_json_full.groups[i].resources[j].path =
                            AdaptPvZ2InternationalResPath.new_res_to_old_res(
                                res_json_full.groups[i].resources[j].path as any
                            );
                    }
                    let origpath_str: string = res_json_full.groups[i]
                        .resources[j].origpath as string;
                    let lambda: any;
                    for (
                        let k: number = 0;
                        k < res_json_full.groups[i].resources[j].path.length;
                        ++k
                    ) {
                        const index: number = origpath_str.indexOf(
                            res_json_full.groups[i].resources[j].path[k]
                        );
                        lambda =
                            index !== -1
                                ? index +
                        ...
                        const sliced_str: string =
                            index !== -1
                                ? origpath_str.slice(
                                      origpath_str.indexOf(
                                          res_json_full.groups[i].resources[j]
                                              .path[k]
                                      ),
                                      lambda
                                  )
                                : origpath_str.slice(lambda);
                        (res_json_full.groups[i].resources[j].path[k] as any) =
                            sliced_str;
                    }
                    delete res_json_full.groups[i].resources[j].origpath;
                    if (!path_iz_list) {
                        res_json_full.groups[i].resources[j].path =
                            AdaptPvZ2InternationalResPath.old_res_to_new_res(
                                res_json_full.groups[i].resources[j].path as any
                            );
                    }
                }
