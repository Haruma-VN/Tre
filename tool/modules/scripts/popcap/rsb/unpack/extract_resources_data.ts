...
export default async function (
    ...
    for (let k = rsg_name_list_uppercase.length - 1; k >= 0; k--) {
        if (
            rsg_name_list_uppercase[k].name_path.indexOf(
                "__MANIFESTGROUP__"
            ) !== -1
        ) {
            for (let i = rsg_list_info.length - 1; i >= 0; i--) {
                if (
                    rsg_name_list_uppercase[k].rsg_pool_index ===
                    rsg_list_info[i].rsg_item_pool_index
                ) {
                    let resources_rsg_package = rsb_buffer_for_unpacking.slice(
                        rsg_list_info[i].rsg_item_offset,
                        rsg_list_info[i].rsg_item_offset +
                            rsg_list_info[i].rsg_item_size
                    );
                    if (
                        ...
                        resources_header_info_repair.writeBuffer(
                            rsg_list_info[i].rsg_temp_info_fixing_rac_rsb
                        );
                        resources_rsg_package.fill(
                    ...
                    if (
                        rsg_list_info[i].rsg_name_item[126] +
                            rsg_list_info[i].rsg_name_item[127] !==
                            0 ||
                        resources_rsg_package.slice(0, 4).toString() !== "pgsr"
                    ) {
                        ...
                        for (let rsg_index in rsg_list_info) {
                            for (let key in rsg_name_list_uppercase) {
                                if (
                                    rsg_name_list_uppercase[key]
                                        .rsg_pool_index ===
                                    rsg_list_info[rsg_index].rsg_item_pool_index
                                ) {
                                    rsg_list_info[rsg_index].rsg_name_item =
                                        rsg_name_list_uppercase[key].name_path;
                                    break;
                                }
                            }
                            const rsg_header_info_repair = new SmartBuffer();
                            rsg_header_info_repair
                                .writeString("pgsr")
                                .writeInt8(4)
                                .writeBuffer(Buffer.alloc(11));
                            rsg_header_info_repair.writeBuffer(
                                rsg_list_info[rsg_index]
                                    .rsg_temp_info_fixing_rac_rsb
                            );
                            ...
                            for (let rsg_name_path in res_rsg_list) {
                                if (
                                    ...
                                    rsg_list_info[
                                        ...
                                        res_rsg_list[rsg_name_path];
                                    res_rsg_list.splice(
                                        rsg_name_path as any,
                                        1
                                    );
                                    break;
                                }
                        ...
                        for (let rsg_item in rsg_list_info) {
                            rsg_list_info[rsg_item].rsg_name_item =
                                rsg_list_info[rsg_item].rsg_name_item
                                    .slice(
                                        0,
                                        rsg_list_info[
                                            rsg_item
                                        ].rsg_name_item.indexOf("\x00")
                                    )
                                    .toString();
                        }
                    }
