"use strict";
export default function (rsgp_head_offset: any, rsb_buffer_for_unpacking: Buffer, rsgp_list_data_info: Buffer) {
    ...
    for (let i = 0; i < rsgp_list_data_info.length; i += 204) {
        ...
        const rsgp_size_length_1 = rsgp_list_item.slice(164, 168).readUInt32LE();
        const rsgp_size_length_2 = rsgp_list_item.slice(168, 172).readUInt32LE();
        const rsgp_item_size = (rsgp_size_length_1 + rsgp_size_length_2);
        if (rsgp_head_offset >= rsb_buffer_length - 4) {
            rsgp_head_offset = 0;
        }
        if (rsb_buffer_for_unpacking.slice(rsgp_head_offset + 76, rsgp_head_offset + 80).readUInt32LE() != 92) {
