...
export default function unpack_rsb(rsb_path: string, resgroup_method: boolean = false, unpack_packages: boolean = false, decode_rton: boolean = false, split_res_json: boolean = false): void {
    ...
    if (head_info.magic !== "1bsr") {
        throw new Error("Not a RSB file");
    }
    if (head_info.version !== 3 && head_info.version !== 4) {
        throw new Error("invalid RSB version");
    };
    ...
    const ptx_info_list = unpack_ptx_info(rsb_b.slice(head_info.ptx_info_begin_offset, head_info.ptx_info_begin_offset + (head_info.ptx_number * head_info.ptx_info_each_length)));
    const mainfest: any = unpack_composite(rsb_composite_b, head_info.version);
    const rsg_info: Buffer = rsb_b.slice(head_info.rsg_info_begin_offset, head_info.rsg_info_begin_offset + (head_info.rsg_number * head_info.rsg_info_each_length));
    mainfest.group = unpack_rsg(mainfest, `${rsb_path}.bundle`, rsg_info, rsb_b, ptx_info_list, resgroup_method, unpack_packages, decode_rton, split_res_json);
