    ...
    for (let i = 0; i < rsg_path_info.length; i++) {
        ...
        if (simple_mode) {
            if (item_ext === ".JSON" && `${rsg_folder}/${rsg_path_info[i].path}`.endsWith(".PAM.JSON")) {
                ...
                const item_data: Buffer = pam_encode(animation_json);
                ...
                if (merge_mode) {
                    rsg_file_data.push({item_data: (json_2_rton(json_data, rsg_default_setting.rton_encrypted, true) as Buffer), ptx_info: null});
                } else {
                    const rton_encrypted: boolean = packet_info.res[rsg_path_info[i].manifest_index] ?? false;
                    rsg_file_data.push({ item_data: (json_2_rton(json_data, rton_encrypted, true) as Buffer), ptx_info: null});
                }
                rsg_path_info[i].path = rsg_path_info[i].path.slice(0, rsg_path_info[i].path.length - item_ext.length) + ".RTON";
                rsg_path_info[i].packet_index = i;
                ...
                const texture_info = encode_texture(fs_js.read_file(`${rsg_folder}/${rsg_path_info[i].path}`, "buffer") as Buffer, merge_mode, merge_mode_ptx_id, rsg_default_setting, texture_format_mapper, (merge_mode ? undefined : packet_info.res[rsg_path_info[i].manifest_index].ptx_info));
                rsg_file_data.push(texture_info);
                rsg_path_info[i].path = rsg_path_info[i].path.slice(0, rsg_path_info[i].path.length - item_ext.length) + ".PTX";
                rsg_path_info[i].packet_index = i;
            } else {
                if (item_ext === ".PTX" && !merge_mode) {
                    if (packet_info.res[rsg_path_info[i].manifest_index].ptx_info === undefined) {
                        throw new Error("expect ptx_info" + rsg_path_info[i].path);
                    } else {
