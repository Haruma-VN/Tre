...
export default function encode_texture(img_buf: Buffer, merge_mode: boolean, merge_mode_ptx_id: [number], rsg_default_setting: rsg_default_setting, texture_format_mapper: any, ptx_info: undefined | {
    id: number,
    ptx_fmt: number,
    ptx_platform: string,
}) {
    ...
    if (merge_mode) {
        ...
        if (ptx_info === undefined) {
            throw new Error("expect ptx_info");
        }
        const fmt_index: number = take_fmt_index(ptx_info.ptx_fmt, ptx_info.ptx_platform);
        const item_data = popcap_texture_encode(img_buf, "", fmt_index, true, true);
        return {item_data, ptx_info: {id: ptx_info.id, width, height}};
    }
}