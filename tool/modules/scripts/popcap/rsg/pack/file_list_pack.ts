...
export default function concat_smart_path(rsg_path_temp: rsg_path_info): path_temp {
    const rsg_path_info: rsg_path_info = [{ path: "", manifest_index: -1, packet_index: -1, ptx_info: null }, ...rsg_path_temp].sort((a, b) => a.path.localeCompare(b.path));
    ...
    for (let i = 0; i < rsg_path_info.length - 1; i++) {
        ...
        if (path_2.length !== Buffer.byteLength(path_2)) {
            throw new Error("item_path_must_be_ascii | " + path_2);
        }
