...
export default function pack_rsg(
    ...
    const rsg_default_setting: rsg_default_setting = fs_js.create_toolkit_view("packets").default_settings as rsg_default_setting;
    ...
    const texture_format_mapper: any = fs_js.read_json(fs_js.dirname(args.main_js as any) + "/extension/settings/texture_format_mapper.json", true);
    {
        if (rsg_default_setting.compression_flags < 0 && rsg_default_setting.compression_flags > 3) {
            throw new Error("compression_flags_out_of_range");
        }
        if (typeof rsg_default_setting.rton_encrypted !== "boolean") {
            throw new Error("setting rton encrypted");
        }
        if (Object.keys(texture_format_mapper).includes(rsg_default_setting.ptx_platform)) {
            if (!Object.keys(texture_format_mapper[rsg_default_setting.ptx_platform]).includes(String(rsg_default_setting.ptx_fmt) as string)) {
                throw new Error("setting ptx fmt");
            }
        } else {
            throw new Error("setting ptx platform");
        }
    }
