...
export default async function (
    ...
    async function ConcatRSGath(filepath: string) {
        ...
        for (let i = 0; i < filepath.length - 1; i++) {
            ...
            const path_length =
                path_orignal.length > path_compare.length ? path_orignal.length : path_compare.length / 4;
    ...
    async function EncodePTX(file_path: string, format: number, format_type: string) {
        if (fs_js.is_file(`${path_file}/Res/ATLASES/${fs_js.parse_fs(file_path).name}.PTX`)) {
            fs_js.js_remove(`${path_file}/Res/ATLASES/${fs_js.parse_fs(file_path).name}.PTX`);
        }
        switch (format) {
            case 0:
                if (format_type === "ios") {
    ...
    async function EncodeJson(file_path: string, rton_2c_encrypted: boolean) {
        if (rton_2c_encrypted) {
            ...
            const rton_data = json2rton(fs_js.read_json(file_path), false);
    ...
    async function GetInfoRSG(file_path: string, UseTreInfo: boolean, TreRSGInfo: any) {
        if (file_path.endsWith(".JSON") && pack_simple && !file_path.endsWith(".PAM.JSON")) {
            ...
            for (let json_key in TreRSGInfo) {
                if (
                    ...
                    TreRSGInfo[json_key].Path[TreRSGInfo[json_key].Path.length - 1]
                ) {
            ...
            if (rsb_pack) {
                if (resources_pack) {
                    const rton_data = await EncodeJson(fs_js.read_json(`${path_file}`) as any, rton_2c_encrypted);
                    return rton_data;
                } else {
                    const rton_data = await EncodeJson(
                        fs_js.read_json(`${fs_js.parse_fs(path_file).dir}/${file_path}`) as any,
                        rton_2c_encrypted,
                    );
                    return rton_data;
                }
            ...
            if (UseTreInfo) {
                ...
                if (file_path.indexOf(".PNG") !== -1) {
                    if (pack_simple) {
                        ...
                        const image_data = await fs_js.read_file(`${path_file}/Res/${file_path}`, "buffer");
                        const image_dimension = await image_util.dimension(`${path_file}/Res/${file_path}`);
                        return {
                            image_data,
                            width: image_dimension.width,
                ...
                if (file_path.indexOf(".PNG") !== -1) {
                    if (pack_simple) {
                        ...
                        const image_dimension = await image_util.dimension(`${path_file}/Res/${file_path}`);
                        if (format_choose === -1) {
                            const allowance_for_popcap_ptx_compression = display_argument(
                                image_dimension.width,
                                image_dimension.height,
                            );
                            const notify_allowance_message = allowance_for_popcap_ptx_compression
                                ? localization("atlas_is_filled_with_2n")
                                : localization("atlas_is_not_filled_with_2n");
                            ...
                            Console.WriteLine(
                                color.fggreen_string(`◉ ${localization("execution_display_width")}: `) +
                                    `${image_dimension.width}`,
                            );
                            Console.WriteLine(
                                color.fggreen_string(`◉ ${localization("execution_display_height")}: `) +
                                    `${image_dimension.height}`,
                            );
                            Console.WriteLine(
                                color.fgcyan_string(
                                    `◉ ${localization("execution_argument")}: ${localization("popcap_ptx_encode")}`,
                                ),
                            );
                            const set_allowance_point = allowance_for_popcap_ptx_compression ? 5 : 2;
                            Console.WriteLine("      1. popcap PTX RGBA8888 Encode (0)");
                            Console.WriteLine("      2. popcap PTX ARGB8888 Encode (0)");
                            if (allowance_for_popcap_ptx_compression) {
                                Console.WriteLine("      3. popcap PTX RGB_PVRTC4_A_8 Encode (30)");
                                Console.WriteLine("      4. popcap PTX RGB_ETC1_A_8 Encode (147)");
                                Console.WriteLine("      5. popcap PTX RGB_ETC1_A_Palette Encode (147)");
                            }
                    ...
                    if (pack_simple) {
                        ...
                        Console.WriteLine(
                            color.fgcyan_string(
                                `◉ ${localization("execution_argument")}: ${localization("popcap_ptx_dimension")}`,
                            ),
                        );
                        Console.WriteLine(
            ...
            if (rsb_pack) {
                if (resources_pack) {
                    ...
                    const file_data = fs_js.read_file(`${fs_js.parse_fs(path_file).dir}/${file_path}`, "buffer");
                    return file_data;
                }
            } else {
                const file_data = fs_js.read_file(`${path_file}/Res/${file_path}`, "buffer");
                return file_data;
            }
        }
    }
    async function PackData(filepath: string, path_temp: any, UseTreInfo: boolean, TreRSGInfo: any) {
        ...
        for (let i = 0; i < filepath.length - 1; i++) {
            ...
            for (let k = 0; k < path_length; k++) {
                if (path_orignal[k] !== path_compare[k]) {
                    rsg_path_info.writeBuffer(path_temp[i].path_compare.toBuffer().slice(k * 4));
                    const rsg_data_info = await GetInfoRSG(path_compare, UseTreInfo, TreRSGInfo);
                    if (rsg_data_info.width !== undefined) {
                        ...
                        rsg_file_data
                            ...
                            .writeBuffer(Buffer.alloc(BeautifyOffset(rsg_data_info.length)));
                    }
                    break;
                }
            }
    ...
    function PackRSG(rsg_path_info: any, rsg_file_data: any, atlas: any, compression: boolean) {
        ...
        const info_header_size = 92 + rsg_path_info.length + BeautifyOffset(92 + rsg_path_info.length);
        ...
        if (atlas) {
            ...
            rsg_data.writeInt32LE(rsg_path_info.length).writeInt32LE(92).writeBuffer(Buffer.alloc(12));
            rsg_data.writeBuffer(rsg_path_info).writeBuffer(Buffer.alloc(BeautifyOffset(92 + rsg_path_info.length)));
            rsg_data.writeString("78da0300", "hex").writeInt32BE(1).writeBuffer(Buffer.alloc(4088));
            rsg_data.writeBuffer(rsg_zlib_data).writeBuffer(Buffer.alloc(BeautifyOffset(rsg_data_length)));
            return rsg_data.toBuffer();
        } else {
            rsg_data.writeInt32LE(rsg_zlib_data.length).writeInt32LE(rsg_file_data.length).writeBuffer(Buffer.alloc(4));
            rsg_data.writeInt32LE(rsg_data_length + BeautifyOffset(rsg_data_length)).writeBuffer(Buffer.alloc(28));
            rsg_data.writeInt32LE(rsg_path_info.length).writeInt32LE(92).writeBuffer(Buffer.alloc(12));
            rsg_data
                .writeBuffer(rsg_path_info)
                .writeBuffer(Buffer.alloc(BeautifyOffset(92 + rsg_path_info.length)))
    ...
    if (TreRSGInfo !== false) {
        ...
        return await PackRSGSimple([rsg_tre_info.CompressionMethod, true], rsg_tre_info.Res);
    } else if (rsb_pack || resources_pack) {
