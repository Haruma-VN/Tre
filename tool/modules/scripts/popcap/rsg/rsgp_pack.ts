...
export default async function (
    ...
    function ZlibDeflate(rsgp_file_data: any, compression: boolean) {
        if (compression) {
            const zlib_deflate = SmartBuffer.fromBuffer(
                zlib.deflateSync(rsgp_file_data, {
                    level: 9,
                })
            );
    ...
    async function EncodeJson(file_path: string, rton_2c_encrypted: boolean) {
        if (rton_2c_encrypted) {
            const rton_cipher_key = fs.readjson(
        ...
        if (
            ...
            for (let json_key in TreRSGPInfo) {
                if (
                    ...
                    TreRSGPInfo[json_key].Path[
                        TreRSGPInfo[json_key].Path.length - 1
                    ]
                ) {
            ...
            if (rsb_pack) {
                if (resources_pack) {
                    const rton_data = await EncodeJson(
                        fs.readjson(`${path_file}`),
                        rton_2c_encrypted
                    );
                    return rton_data;
                } else {
                    const rton_data = await EncodeJson(
                        fs.readjson(`${parse(path_file).dir}/${file_path}`),
                        rton_2c_encrypted
                    );
                    return rton_data;
                }
                ...
                const rton_data = await EncodeJson(
                    fs.readjson(`${path_file}/Res/${file_path}`),
                    rton_2c_encrypted
                );
                return rton_data;
            }
            ...
            if (UseTreInfo) {
                ...
                for (let key in TreRSGPInfo) {
                    if (
                        parse(file_path).base.toUpperCase() ===
                        TreRSGPInfo[key].Path[TreRSGPInfo[key].Path.length - 1]
                    ) {
                ...
                if (file_path.indexOf(".PNG") !== -1) {
                    if (pack_simple) {
                        ...
                        const image_data = await fs.readfilebuffer(
            ...
            if (rsb_pack) {
                if (resources_pack) {
                    ...
                    const file_data = fs.readfilebuffer(
        ...
        for (let i = 0; i < filepath.length - 1; i++) {
            ...
            for (let k = 0; k < path_length; k++) {
                if (path_orignal[k] !== path_compare[k]) {
                    ...
                    const rsgp_data_info = await GetInfoRSGP(
                        path_compare,
                        UseTreInfo,
                        TreRSGPInfo
                    );
                    if (rsgp_data_info.width !== undefined) {
                        rsgp_path_info
                            .writeInt32LE(1)
                            .writeInt32LE(rsgp_file_data.writeOffset)
                            .writeInt32LE(rsgp_data_info.image_data.length);
                        rsgp_data_info.id !== false
                            ? rsgp_path_info.writeInt32LE(rsgp_data_info.id)
                            : rsgp_path_info.writeInt32LE((PTX_id_index += 1));
                        rsgp_path_info
                            .writeBuffer(Buffer.alloc(8))
                            .writeInt32LE(rsgp_data_info.width)
                            .writeInt32LE(rsgp_data_info.height);
                        ...
                        rsgp_path_info
                            ...
                            .writeInt32LE(rsgp_file_data.writeOffset)
                            .writeInt32LE(rsgp_data_info.length);
                        rsgp_file_data
                            .writeBuffer(rsgp_data_info)
                            .writeBuffer(
                                Buffer.alloc(
                                    BeautifyOffset(rsgp_data_info.length)
                                )
                            );
                    }
                    break;
    ...
    function PackRSGP(
        ...
        const info_header_size =
            92 +
            rsgp_path_info.length +
            BeautifyOffset(92 + rsgp_path_info.length);
        ...
        if (atlas) {
            ...
            rsgp_data
                ...
                .writeInt32LE(rsgp_zlib_data.length)
                .writeInt32LE(rsgp_file_data.length)
                .writeBuffer(Buffer.alloc(20));
            rsgp_data
                .writeInt32LE(rsgp_path_info.length)
                .writeInt32LE(92)
                .writeBuffer(Buffer.alloc(12));
            rsgp_data
                .writeBuffer(rsgp_path_info)
                .writeBuffer(
                    Buffer.alloc(BeautifyOffset(92 + rsgp_path_info.length))
                );
            rsgp_data
            ...
            rsgp_data
                .writeBuffer(rsgp_zlib_data)
                .writeBuffer(Buffer.alloc(BeautifyOffset(rsgp_data_length)));
            ...
            rsgp_data
                .writeInt32LE(
                    rsgp_data_length + BeautifyOffset(rsgp_data_length)
                )
            ...
            rsgp_data
                ...
                .writeBuffer(rsgp_zlib_data);
            rsgp_data.writeBuffer(
                Buffer.alloc(BeautifyOffset(rsgp_data_length))
            );
            return rsgp_data.toBuffer();
        }
    }
    async function PackRSGPSimple(TreInfo: any, TreRSGPInfo: any) {
        ...
        const [rsgp_path_info, rsgp_file_data, atlas] = await PackData(
            filepath as any,
            path_temp,
            TreInfo[1],
            TreRSGPInfo
        );
        return await PackRSGP(
            rsgp_path_info,
            rsgp_file_data,
            atlas,
            TreInfo[0]
    ...
    if (TreRSGPInfo !== false) {
        ...
        return await PackRSGPSimple(
            [rsgp_tre_info.CompressionMethod, true],
            rsgp_tre_info.Res
        );
    } else if (rsb_pack || resources_pack) {
