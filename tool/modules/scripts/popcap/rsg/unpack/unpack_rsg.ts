...
export default function unpack_rsg(
    rsg_input: string,
    rsg_b: Buffer,
    simple_mode: boolean | { decode_rton: boolean; decode_pam: boolean; decode_bnk: boolean; decode_texture: boolean } = false,
    return_mode: boolean = false,
    disable_notify: boolean = false
): void | { rsg_temp_list: Array<{ path: string; decode_data: Buffer | null; json_data: any }>; packet_info: packet_info } {
    ...
    if (head_info.version !== 3 && head_info.version !== 4) {
        throw new Error("invalid rsg version");
    }
    if (0 < flags && flags > 3) {
        throw new Error("invalid rsg flags");
    }
    if (simple_mode === true && head_info.version === 3) {
        throw new Error("simple mode only support version 4 (pvz2)");
    }
    ...
    function choose_type(decode_data: Buffer | null, json_data: Buffer | null, item_path: string): void {
        if (decode_data !== null && json_data === null) {
            ...
            throw new Error("invalid decode_data or json_data");
        }
    }
    if (part_0_list.length > 0) {
        ...
        for (let i = 0; i < part_0_list.length; i++) {
            ...
            const path: string | Array<string> = array_path ? packet_info_list.item_path.split("\\") : packet_info_list.item_path;
            packet_info_list.rton_encrypted ? packet_info.res.push({ rton_encrypted: packet_info_list.rton_encrypted, path }) : packet_info.res.push({ path });
        }
    }
    const part_1_list: Array<{ path: string; offset: number; size: number; id: number; width: number; height: number }> = file_list.part_1_list;
    if (part_1_list.length > 0) {
        for (let i = 0; i < part_1_list.length; i++) {
            ...
            for (let i = 0; i < part_1_list.length; i++) {
                ...
                const path: string | Array<string> = array_path ? texture_info_list.item_path.split("\\") : texture_info_list.item_path;
                typeof texture_info_list.ptx_fmt === "boolean"
                    ? packet_info.res.push({ path, ptx_info: { id: part_1_list[i].id, width: part_1_list[i].width, height: part_1_list[i].height } })
                    : packet_info.res.push({
                          path,
                          ptx_info: { id: part_1_list[i].id, ptx_fmt: texture_info_list.ptx_fmt, ptx_platform: texture_info_list.ptx_platform },
                      });
            }
        }
    }
