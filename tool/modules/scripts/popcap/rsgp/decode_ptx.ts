...
export default async function DecodePTX(ptx_path: string, file_data_size: number, image_width: number, image_height: number, ios_argb8888: number | boolean,
    ...
    switch (square_ratio) {
        case 20:
            ...
            if (ios_argb8888 == 1) {
                await image_util.decode_argb8888(ptx_path, image_width, image_height, no_notify_message);
                format = { Format: 0, Platform: 'ios' };
            }
            else {
                await image_util.decode_rgba8888(ptx_path, image_width, image_height, no_notify_message);
                format = { Format: 0, Platform: 'android' };
            }
            break;
        case 2:
            await image_util.decode_pvrtc(ptx_path, image_width, image_height, no_notify_message);
            format = { Format: 30, Platform: 'ios' };
            break;
        case 5:
            await image_util.decode_etc1alpha_palette(ptx_path, image_width, image_height, no_notify_message);
            format = { Format: 147, Platform: 'android_cn' };
            break;
        case 7:
            await image_util.decode_etc1a(ptx_path, image_width, image_height, no_notify_message);
