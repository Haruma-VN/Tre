...
export default async function (
    ...
    let pam_to_xfl: boolean = fs_js.create_toolkit_view(
        "pam_to_flash"
    ) as boolean;
    let pam_resolution: number = fs_js.create_toolkit_view(
        "pam_resolution"
    ) as number;
    let treRSGPinfo: any = {
        UseTreRSGPInfo: true,
    ...
    async function DecodeRTON(rton_data: any) {
        ...
        if (rton_head === "1000") {
            ...
            const rton_plain_data = await rton_plain(
                rton_data,
                rton_cipher_key
            );
    ...
    async function WriteFile(
        ...
        const file_data_offset = rsgp_data
            .slice(temp_offset - 8, temp_offset - 4)
            .readInt32LE();
        ...
        if (atlas) {
            ...
            const image_height = rsgp_data
                ...
                .readInt32LE();
            const ptx_data = rsgp_file_data.slice(
                file_data_offset,
                file_data_offset + file_data_size
            );
            await fs.outfile(`${rsgp_path}/Res/${name_path}`, ptx_data);
            if (decode_ptx) {
                const format = await DecodePTX(
                    `${rsgp_path}/Res/${name_path}`,
                    file_data_size,
                    image_width,
                    image_height,
                    ios_argb8888,
            ...
            fs.outfile(
                `${rsgp_path}/Res/${parse(name_path).dir}/${
                    parse(name_path).name
                }.json`,
                json_data[0]
            );
            ...
            if (pam_to_xfl) {
                await pam_xfl_decode(
                    pam_data,
                    `${rsgp_path}/Res/${name_path}.json`,
                    pam_resolution
                );
                pam_path[pam_path.length - 1] =
                    pam_path[pam_path.length - 1] + ".XFL";
            } else {
                pam_path[pam_path.length - 1] =
                    pam_path[pam_path.length - 1] + ".JSON";
                fs.outfile(
                    `${rsgp_path}/Res/${name_path}.json`,
                    Buffer.alloc(64)
                );
            ...
            let pam_path = name_path.split("\\");
            pam_path[pam_path.length - 1] =
                pam_path[pam_path.length - 1] + ".SOUNDBANK";
            ...
            const file_data = rsgp_file_data.slice(
                file_data_offset,
                file_data_offset + file_data_size
            );
    ...
    async function Unpack_Rsgp(
        offset: number,
        size: number,
        atlas: boolean,
        compression: boolean
    ) {
        rsgp_file_data = compression
            ? zlib.unzipSync(rsgp_data.slice(offset, offset + size))
            : rsgp_data.slice(offset, offset + size);
    ...
    const zlib_compression = await CheckZlibHeader(
        rsgp_data.slice(part0_Offset, part0_Offset + 2)
    );
    async function CheckCompression() {
        if (
            (part0_ZSize === part0_Size &&
                part0_Size !== 0 &&
                zlib_compression) ||
            (part1_ZSize === part1_Size &&
                part1_ZSize !== 0 &&
                zlib_compression) ||
            ...
            part1_ZSize !== 0
                ? await Unpack_Rsgp(part1_Offset, part1_ZSize, true, false)
                : await Unpack_Rsgp(part0_Offset, part0_ZSize, false, false);
        } else {
            part1_ZSize !== 0
                ? await Unpack_Rsgp(part1_Offset, part1_ZSize, true, true)
                : await Unpack_Rsgp(part0_Offset, part0_ZSize, false, true);
        }
    }
