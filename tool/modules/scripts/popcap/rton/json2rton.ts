...
export default function (rton_data: any): any {
    ...
    function EncodeIntNumber(int: any) {
        if (int === 0) {
            ...
            json_data
                .writeString("25", "hex")
                .writeBuffer(RtonNumber(int, true));
    ...
    function EncodeUTF8String(string: string) {
        json_data
            .writeBuffer(RtonNumber(Buffer.byteLength(string) as any))
            .writeBuffer(RtonNumber(Buffer.byteLength(string) as any))
            .writeString(string);
    }
    function EncodeRTID(value: any) {
        if (value.includes("@")) {
            ...
            if ((name_str.match(/\./g) || []).length === 2) {
                ...
                json_data
                    .writeBuffer(RtonNumber(int2))
                    .writeBuffer(RtonNumber(int1))
                    .writeBuffer(Buffer.from(int3, "hex").reverse());
            } else {
    ...
    function EncodeCacheString(string: string) {
        if (string in cached_strings) {
            json_data
                .writeString("91", "hex")
                .writeBuffer(RtonNumber(cached_strings[string]));
        } else {
    ...
    function EncodeArray(array_value: any) {
        json_data
            .writeString("86fd", "hex")
            .writeBuffer(RtonNumber(array_value.length));
