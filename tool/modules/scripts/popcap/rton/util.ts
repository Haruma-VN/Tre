...
export function popcap_rton_to_json(
    ...
    if (fs.statSync(filepath).isDirectory()) {
        ...
        outfile(
            ...
            rton2json(readfilebuffer(filepath)) as any
        );
        if (!this_will_stop_console_log) {
            console.log(
                `${color.fggreen_string(
...
export function popcap_json_to_rton_and_encrypt(
    ...
    if (fs.statSync(filepath).isDirectory()) {
        ...
        for (let file of folder) {
            if (parse(file).ext.toUpperCase() === ".JSON") {
                ...
                outfile(
                    jsonpath,
                    rton_cipher(
                        json2rton(readjson(file)) as any,
                        rton_cipher_key
                    )
                );
            }
        ...
        outfile(
            ...
            rton_cipher(json2rton(readjson(filepath)) as any, rton_cipher_key)
        );
        if (!this_will_stop_console_log) {
            console.log(
                `${color.fggreen_string(
...
export function popcap_json_to_rton(
    ...
    if (fs.statSync(filepath).isDirectory()) {
        ...
        outfile(
            `${parse(filepath).dir}/${parse(filepath).name}.rton`,
            json2rton(readjson(filepath)) as any
        );
        if (!this_will_stop_console_log) {
            console.log(
                `${color.fggreen_string(
